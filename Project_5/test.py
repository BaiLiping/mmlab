...
def parse_args():
    ...
    parser.add_argument('--out_dir', help='output result file in pickle format')
    parser.add_argument('--data_file', help='wehre to find the point cloud data')
...
def removesuffix(input_string, suffix):
    return input_string[:len(input_string)-len(suffix)]
...
def main():
    ...
    pcd_list=glob.glob(path.join(args.data_file,'*.pcd.bin'))
    for i in tqdm(range(len(pcd_list))):
    #for pcd in pcd_list:
        pcd=pcd_list[i]
        bboxes_3d, labels_3d=inference(model,pcd)
        pcd=removeprefix(pcd, args.data_file+'/')
        pcd=removesuffix(pcd, '.pcd.bin')
        ...
        #print(labels_3d)
    inference_out = open(path.join(args.out_dir,'/inferecen_result.pkl'),'wb')
